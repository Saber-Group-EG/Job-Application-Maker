...
function normalizeJobFieldConfig(
  value: any,
  ...
  return {
    fullName: normalizeFieldConfigRule(
      withFallbackExpectedSalary.fullName,
      DEFAULT_FIELD_CONFIG.fullName
    ),
    email: normalizeFieldConfigRule(
      withFallbackExpectedSalary.email,
      DEFAULT_FIELD_CONFIG.email
    ),
    phone: normalizeFieldConfigRule(
      withFallbackExpectedSalary.phone,
      DEFAULT_FIELD_CONFIG.phone
    ),
    gender: normalizeFieldConfigRule(
      withFallbackExpectedSalary.gender,
      DEFAULT_FIELD_CONFIG.gender
    ),
    birthDate: normalizeFieldConfigRule(
      withFallbackExpectedSalary.birthDate,
      DEFAULT_FIELD_CONFIG.birthDate
    ),
    address: normalizeFieldConfigRule(
      withFallbackExpectedSalary.address,
      DEFAULT_FIELD_CONFIG.address
    ),
    profilePhoto: normalizeFieldConfigRule(
      withFallbackExpectedSalary.profilePhoto,
      DEFAULT_FIELD_CONFIG.profilePhoto
    ),
    cvFilePath: normalizeFieldConfigRule(
      withFallbackExpectedSalary.cvFilePath,
      DEFAULT_FIELD_CONFIG.cvFilePath
    ),
    expectedSalary: normalizeFieldConfigRule(
      withFallbackExpectedSalary.expectedSalary,
      DEFAULT_FIELD_CONFIG.expectedSalary
    ),
  };
}
...
class JobPositionsService {
  ...
  public normalizeJobPosition(maybe: any): JobPosition {
    ...
    if (Array.isArray(maybe.jobSpecsWithDetails)) {
      maybe.jobSpecsWithDetails = maybe.jobSpecsWithDetails.map(
        (d: any, idx: number) => ({
    ...
    const rawSpecs: any[] = Array.isArray(maybe.jobSpecs)
      ? [...maybe.jobSpecs]
      : [];
    const responses: any[] = Array.isArray(maybe.jobSpecsResponses)
      ? [...maybe.jobSpecsResponses]
      : [];

    // Handle alternative response shapes
    if (Array.isArray(maybe.job_specs_with_details))
      rawSpecs.push(...maybe.job_specs_with_details);
    if (Array.isArray(maybe.jobSpecsWithDetail))
      rawSpecs.push(...maybe.jobSpecsWithDetail);
    ...
    const specs = rawSpecs.map((s: any, idx: number) => ({
      ...
      spec:
        s.spec ??
        (s.spec && typeof s.spec === 'object' ? (s.spec.en ?? '') : s.spec) ??
        '',
      weight:
  ...
  private buildJobPositionPayload(
    data: Partial<CreateJobPositionRequest | UpdateJobPositionRequest>
  ): any {
    const payload: any = {};
    ...
    if (data.openPositions !== undefined)
      payload.openPositions = data.openPositions;
    ...
    if (Array.isArray(data.allowedStatuses))
      payload.allowedStatuses = data.allowedStatuses;
  ...
  async reorderJobPositions(
    ...
    await Promise.all(
      normalizedItems.map((item) => {
        ...
        return this.updateJobPosition(item.id, {
          ...basePayload,
          order: item.order,
        });
      })
