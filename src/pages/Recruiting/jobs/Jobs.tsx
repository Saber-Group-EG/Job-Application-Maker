      ...
      {filteredJobs.length === 0 ? (
        ...
        <>
          ...
          >
            {jobsGroupedByCompany.map((group: any) => {
              ...
              return (
                <div key={group.companyId} className="space-y-4">
                  ...
                  <SortableContext items={groupJobIds} strategy={rectSortingStrategy}>
            ...
            <DragOverlay>
              {activeDragJob ? (
                <div className="w-[calc(100vw-3rem)] max-w-[28rem] space-y-4 rounded-3xl border border-white/20 bg-white/90 p-6 shadow-2xl dark:border-slate-800/50 dark:bg-slate-900/90">
                  <div className="flex items-start justify-between">
                    <div className="space-y-1">
                      <div className="flex items-center gap-2">
                        <span className={`inline-flex rounded-full px-2.5 py-0.5 text-[10px] font-bold uppercase tracking-wider ${
                          activeDragJob.isActive !== false ? "bg-emerald-100 text-emerald-700 dark:bg-emerald-500/10 dark:text-emerald-400" :
                          "bg-slate-100 text-slate-700 dark:bg-slate-500/10 dark:text-slate-400"
                        }`}>
                          {activeDragJob.isActive !== false ? "Active" : "Deprioritized"}
                        </span>
                      </div>
                  ...
                  <div className="space-y-2.5">
                    <div className="flex items-center gap-2 text-sm text-slate-600 dark:text-slate-400">
                      <Building2Icon className="size-4 text-brand-500" />
                      <span className="font-medium truncate">{getTranslation(activeDragJob.companyId?.name) || "Global Corp"}</span>
                    </div>
                    <div className="flex items-center gap-2 text-sm text-slate-500">
                      <MapPinIcon className="size-4" />
                      <span>{activeDragJob.workArrangement || "Remote / Office"}</span>
                    </div>
                  </div>
                </div>
              ) : null}
