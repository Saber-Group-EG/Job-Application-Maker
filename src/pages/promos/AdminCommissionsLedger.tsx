...
import type { PromoCommission } from '../../types/promos';
...
export default function AdminCommissionsLedger() {
  ...
  const handleSettle = async () => {
    setIsSettleOpen(false); // close modal before ANY Swal, including validation errors
    ...
    if (!settleMonth) {
      ...
      setIsSettleOpen(true); // reopen so they can fix it
      return;
    }
    ...
    if (!result.isConfirmed) {
      setIsSettleOpen(true); // reopen if they backed out
      return;
    }
    ...
    try {
      ...
      setSettleHr('all');
      // note: modal already closed — don't reopen on success
    } catch {
      setIsSettleOpen(true); // reopen so they can retry after the error toast
  ...
  return (
    <div className="min-h-screen bg-[#F8FAFC] dark:bg-[#0F172A] p-4 sm:p-8 text-slate-900 dark:text-slate-100">
      <PageMeta
        title={t('commissionsLedgerMetaTitle', 'promos')}
        description={t('commissionsLedgerSubtitle', 'promos')}
      />

      <div className="max-w-7xl mx-auto space-y-8">
        <PageBreadcrumb
          pageTitle={t('commissionsLedgerTitle', 'promos')}
          actions={
            <button
              onClick={() => setIsSettleOpen(true)}
              className="flex items-center gap-2 px-6 py-3 bg-amber-500 text-white rounded-[1.25rem] font-bold shadow-xl shadow-amber-500/20 hover:scale-105 active:scale-95 transition-all"
            >
